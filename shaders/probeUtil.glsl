...
vec3 sample_irradiance(vec3 world_position, vec3 normal, vec3 camera_position) {
  const vec3 Wo = normalize(camera_position.xyz - world_position); // Vector from World position to camera
  ...
  vec3 biased_world_position = world_position + bias_vector; // Avoid sampling shadows from the surface itself
  ...
  vec3  base_probe_world_position = grid_indices_to_world_no_offsets(base_grid_indices); // Calculate the actual position based on the base_grid_indices

  // alpha is how far from the floor(currentVertex) position. on [0, 1] for each axis.
  vec3 alpha = clamp((biased_world_position - base_probe_world_position), vec3(0.0f), vec3(1.0f)); // For trilinear interpolation
